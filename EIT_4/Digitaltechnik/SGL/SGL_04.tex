...
\subsection{Describing Machines: State Chart Diagrams}
...
\subsection{The State Chart Diagram}
\begin{figure}[h]
\centering
\begin{tikzpicture}[->, >=Stealth, node distance=5cm, every node/.style={circle, draw, minimum size=2.8cm, align=center, line width=1pt}]

    \node[fill=green!10] (S0) {$S_0$ \\ \textbf{GREEN} \\ $Y=001$};
    \node[fill=yellow!10] (S1) [right of=S0] {$S_1$ \\ \textbf{YELLOW} \\ $Y=010$};
    \node[fill=red!10] (S3) [below of=S1] {$S_3$ \\ \textbf{RED} \\ $Y=100$};
    \node[fill=orange!10] (S2) [below of=S0] {$S_2$ \\ \textbf{RED-YELLOW} \\ $Y=110$};

    \path[line width=1pt]
        (S0) edge [loop above] node {Button not \\ pressed ($X=0$)} (S0)
        (S0) edge node [above] {Button pressed \\ ($X=1$)} (S1)
        (S1) edge node [right] {Timer done \\ (always)} (S3)
        (S3) edge [loop below] node {Button still \\ active ($X=1$)} (S3)
        (S3) edge node [below] {Released \\ ($X=0$)} (S2)
        (S2) edge node [left] {Ready \\ (always)} (S0);
\end{tikzpicture}
\end{figure}
